...
use std::env;
use todo::{domain::entities::Repository, infrastructure::infrastructure::DynamoDbRepository};
...
async fn main() -> Result<(), Error> {
    ...
    let _res = run(service_fn(
        |request: LambdaEvent<ApiGatewayV2httpRequest>| get_todo(&repository, request),
    ))
    .await;
